...
def test_recognizes_equipment_by_keyword_in_outfit() -> None:
    match = recognize_gear(["white silver jacket", "black boots"], STORE)
...
def test_ignores_non_equipment_entities() -> None:
    ...
    assert recognize_gear(["example job outfit"], STORE) is None


def test_no_match_returns_none() -> None:
    assert recognize_gear(["plain red dress"], STORE) is None


def test_prefers_more_keyword_hits() -> None:
    match = recognize_gear(["silver jacket and cargo pants"], STORE)
    assert match is not None
    assert match.hits == 2
